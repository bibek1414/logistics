...
import type { SaleItem, DashLocation } from "@/types/sales";
...
export function DashLocationCell({
  ...
  const handleLocationSelect = async (location: DashLocation) => {
    ...
    try {
      await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/sales/orders/${sale.id}/`,
        {
          method: "PATCH",
          headers: {
  ...
  const handleSendOrder = async () => {
    ...
    try {
      await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/dash/send-order/${sale.id}/`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
